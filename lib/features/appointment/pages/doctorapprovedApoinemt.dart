...
class DoctorApprovedAppointments extends StatefulWidget {
  const DoctorApprovedAppointments({Key? key}) : super(key: key);

  @override
  State<DoctorApprovedAppointments> createState() =>
      _DoctorApprovedAppointmentsState();
}

class _DoctorApprovedAppointmentsState
    extends State<DoctorApprovedAppointments> {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<AppointmentsBloc, AppointmentsState>(
      buildWhen: (previous, current) =>
          current.approvedAppointments != previous.approvedAppointments,
      builder: (context, state) {
        return SafeArea(
            child: state.approvedAppointments.isEmpty
                ? Center(
                    child: Text(
                      'لا توجد مواعيد',
                ...
                : ListView.builder(
                    ...
                    shrinkWrap: true,
                    itemCount: state.approvedAppointments.length,
                    itemBuilder: (context, index) {
                      DocumentSnapshot document =
                          state.approvedAppointments[index];
                      return Card(
                        elevation: 2,
                        child: InkWell(
                          onTap: () {},
